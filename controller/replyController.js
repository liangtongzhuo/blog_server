...
const { getRequestWechat } = require('./wechatyController');
...
const reply = async (message, ctx) => {
  ...
  if (message.MsgType == 'text') {
    ...
    // 调用其其它服务器进行微信登录
      ...
      const {code, url} = await getRequestWechat();
      if (code != 0) return '网络错误，二维码获取失败'
      return [
        {
          title: '微信登录二维码',
          description: '扫码登录',
    ...
    // 请求第三方 AI 语句
    else return await postRequest(message.FromUserName, message.Content);
  }
  return '不听不听\n回复「吃饭」，选择餐馆 :)';
};
